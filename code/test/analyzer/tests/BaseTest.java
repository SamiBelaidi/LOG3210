...
public class BaseTest {
    ...
    public void prepare() throws Exception {
        // Prepare
        String name = m_file.getName().substring(0, m_file.getName().length() - 3);
        String path = m_file.getParentFile().getParent();

        Path expectedPath = Paths.get(path + "/expected/" + name + ".py");
        Path resultPath = Paths.get(path + "/result/" + name + ".py");
